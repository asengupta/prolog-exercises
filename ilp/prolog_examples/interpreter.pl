...
interpret(mvc(reg(ToRegister),Value),NextIP,_,vmState(NextIP,Stack,CallStack,Registers,Flag),vmState(NextIP,Stack,CallStack,UpdatedRegisters,Flag)) :- 
                                                        writeln('In mvc' + ToRegister + Registers),
                                                        update_reg(-(reg(ToRegister),Value),Registers,UpdatedRegisters).
interpret(cmp(reg(CmpRegister),CmpValue),NextIP,_,vmState(NextIP,Stack,CallStack,Registers,_),vmState(NextIP,Stack,CallStack,Registers,UpdatedFlag)) :- 
                                                        writeln('In cmp' + CmpRegister + Registers),
                                                        get2(CmpRegister,Registers,RegisterValue),
                                                        equate(RegisterValue,CmpValue,UpdatedFlag).

interpret(j(label(Label)),_,[LabelMap],vmState(_,Stack,CallStack,Registers,Flag),vmState(UpdatedIP,Stack,CallStack,Registers,Flag)) :- 
                                                        writeln('In jmp direct label' + Label + Registers),
                                                        get2(label(Label),LabelMap,UpdatedIP).

interpret(j(reg(JumpRegister)),NextIP,[LabelMap],vmState(NextIP,Stack,CallStack,Registers,Flag),StateOut) :- 
                                                        writeln('In jmp indirect' + JumpRegister + Registers),
                                                        get2(JumpRegister,Registers,RegisterValue),
                                                        interpret(j(RegisterValue),NextIP,[LabelMap],vmState(NextIP,Stack,CallStack,Registers,Flag),StateOut).

interpret(j(JumpIP),_,_,vmState(_,Stack,CallStack,Registers,Flag),vmState(JumpIP,Stack,CallStack,Registers,Flag)) :- writeln('In jmp direct' + JumpIP + Registers).

interpret(jz(reg(JumpRegister)),NextIP,[LabelMap],vmState(NextIP,Stack,CallStack,Registers,Flag),StateOut) :- 
                                                        writeln('In JZ indirect reg' + JumpRegister + Registers),
                                                        get2(JumpRegister,Registers,RegisterValue),
                                                        interpret(jz(RegisterValue),NextIP,[LabelMap],vmState(NextIP,Stack,CallStack,Registers,Flag),StateOut).

interpret(jnz(reg(JumpRegister)),NextIP,[LabelMap],vmState(NextIP,Stack,CallStack,Registers,Flag),StateOut) :- 
                                                        writeln('In JNZ indirect reg' + JumpRegister + Registers),
                                                        get2(JumpRegister,Registers,RegisterValue),
                                                        interpret(jnz(RegisterValue),NextIP,[LabelMap],vmState(NextIP,Stack,CallStack,Registers,Flag),StateOut).

interpret(jz(label(Label)),NextIP,[LabelMap],vmState(NextIP,Stack,CallStack,Registers,Flag),StateOut) :- 
                                                        writeln('In JZ label' + Label + Registers),
                                                        get2(label(Label),LabelMap,JumpIP),
                                                        interpret(jz(JumpIP),NextIP,[LabelMap],vmState(NextIP,Stack,CallStack,Registers,Flag),StateOut).

interpret(jnz(label(Label)),NextIP,[LabelMap],vmState(NextIP,Stack,CallStack,Registers,Flag),StateOut) :- 
                                                        writeln('In JNZ label' + Label + Registers),
                                                        get2(label(Label),LabelMap,JumpIP),
                                                        interpret(jnz(JumpIP),NextIP,[LabelMap],vmState(NextIP,Stack,CallStack,Registers,Flag),StateOut).

interpret(jz(JumpIP),OldNextIP,_,vmState(OldNextIP,Stack,CallStack,Registers,Flag),vmState(UpdatedIP,Stack,CallStack,Registers,Flag)) :- interpret_condition(OldNextIP,JumpIP,Flag,isZero,UpdatedIP).
interpret(jnz(JumpIP),OldNextIP,_,vmState(OldNextIP,Stack,CallStack,Registers,Flag),vmState(UpdatedIP,Stack,CallStack,Registers,Flag)) :- interpret_condition(OldNextIP,JumpIP,Flag,isNotZero,UpdatedIP).

interpret(inc(reg(Register)),NextIP,_,vmState(NextIP,Stack,CallStack,Registers,Flag),vmState(NextIP,Stack,CallStack,UpdatedRegisters,Flag)) :- interpret_update_reg(reg(Register),plusOne,Registers,UpdatedRegisters).
interpret(dec(reg(Register)),NextIP,_,vmState(NextIP,Stack,CallStack,Registers,Flag),vmState(NextIP,Stack,CallStack,UpdatedRegisters,Flag)) :- interpret_update_reg(reg(Register),minusOne,Registers,UpdatedRegisters).
interpret(mul(reg(LHSRegister),reg(RHSRegister)),NextIP,_,vmState(NextIP,Stack,CallStack,Registers,Flag),vmState(NextIP,Stack,CallStack,UpdatedRegisters,Flag)) :- 
...
interpret(term(String),_,_,vmState(NextIP,Stack,CallStack,Registers,Flag),vmState(NextIP,Stack,CallStack,Registers,Flag)) :- writeln(String).
interpret(label(String),_,_,vmState(NextIP,Stack,CallStack,Registers,Flag),vmState(NextIP,Stack,CallStack,Registers,Flag)) :- writeln('ENTER: ' + String ).

interpret(push(reg(Register)),NextIP,_,vmState(NextIP,Stack,CallStack,Registers,Flag),vmState(NextIP,UpdatedStack,CallStack,Registers,Flag)) :- 
                    get2(Register,Registers,RegisterValue),
                    push_(RegisterValue,Stack,UpdatedStack).

interpret(pop(reg(Register)),NextIP,_,vmState(NextIP,Stack,CallStack,Registers,Flag),vmState(NextIP,UpdatedStack,CallStack,UpdatedRegisters,Flag)) :- 
                                                                pop_(Stack,PoppedValue,UpdatedStack),
                                                                update_reg(-(reg(Register),PoppedValue),Registers,UpdatedRegisters).
interpret(push(V),NextIP,_,vmState(NextIP,Stack,CallStack,Registers,Flag),vmState(NextIP,UpdatedStack,CallStack,Registers,Flag)) :- push_(V,Stack,UpdatedStack).

interpret(call(label(LabelName)),NextIP,[LabelMap],vmState(NextIP,Stack,CallStack,Registers,Flag),vmState(CallIP,Stack,UpdatedCallStack,Registers,Flag)) :- 
...
interpret(ret,_,_,vmState(_,Stack,CallStack,Registers,Flag),vmState(PoppedValue,Stack,UpdatedCallStack,Registers,Flag)) :- 
                                                            pop_(CallStack,PoppedValue,UpdatedCallStack),
                                                            writeln('Returning from call...IP is ' + PoppedValue).
interpret(nop,_,_,vmState(NextIP,Stack,CallStack,Registers,Flag),vmState(NextIP,Stack,CallStack,Registers,Flag)).
